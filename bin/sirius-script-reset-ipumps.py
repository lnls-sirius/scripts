...
def conv_ipump_2_controller(ion_pump):
    """."""
    for controller, channels in CONTROLLERS.items():
        if ion_pump in channels:
            channel_idx = channels.index(ion_pump)
            return controller, channel_idx
...
def create_controllers_steps_pvs(controllers, timeout=10):
    pvs = {controller:epics.PV(controller + ':Step-SP') for controller in controllers}
    for pv in pvs.values():
        if not pv.wait_for_connection(timeout=timeout):
            print('timeout in {}'.format(pv.pvname))
    return pvs

def create_ion_pumps_currents_pvs(ion_pumps, timeout=10):
...
# controller_name = 'SI-RA01:VA-SIPC-06'
# ion_pumps = CONTROLLERS[controller_name]
# print(ion_pumps)

controller_name, channel_idx = conv_ipump_2_controller(ION_PUMPS[0])
pvs = create_controllers_steps_pvs(CONTROLLERS)
pv = pvs[controller_name]
print(pv.value)
